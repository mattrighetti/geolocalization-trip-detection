...
class Testing(unittest.TestCase):
    ...
    def test_loading_dataset(self):
        ...
        geo_dataframe = self.s.dataset

        # Check that the initialized dataframe is not empty
        self.assertIsInstance(geo_dataframe, geopandas.geodataframe.GeoDataFrame)
        print("Test passed.\n")

    def test_find_bus_stops_close_to_Duomo(self):
        print("Checking the find bus stops close to Duomo, Milan")
        x = 9.18951
        y = 45.46427
        p = Point(x, y)
        radius = 0.0015
        closest_stops = self.s.find_bus_stops_close_to(p, radius)

        # Check that there are some stops close to the Duomo
        self.assertGreater(len(closest_stops), 0)
        ...
        self.assertTrue(all(is_the_bus_line_contained))
        print("Test passed.\n")

    def test_find_bus_stops_close_to_Porta_Genova(self):
    ...
    def test_find_bus_stops_close_to_Politecnico(self):
    ...
    def test_intercept_from_Duomo_to_Politecnico(self):
        ...
        radius = 0.0015
        duomo_stops = self.s.find_bus_stops_close_to(duomo_coordinate, radius)
        polimi_stops = self.s.find_bus_stops_close_to(politecnico_coordinate, radius)
        relevant_duomo_stops, relevant_politecnico_stops = intercept(duomo_stops, polimi_stops)

        # Check the format of the output
        self.assertIsInstance(relevant_duomo_stops, geopandas.geodataframe.GeoDataFrame)
        self.assertIsInstance(relevant_politecnico_stops, geopandas.geodataframe.GeoDataFrame)
        ...
        does_result_contains_the_expected_columns = [column in relevant_duomo_stops.columns
                                                     for column
                                                     in expected_columns]
        self.assertTrue(all(does_result_contains_the_expected_columns))
        ...
        self.assertEqual(len(duomo_lines), 1)
        self.assertEqual(len(politecnico_lines), 1)
        self.assertEqual(duomo_lines, politecnico_lines)
        self.assertIn('19', politecnico_lines)
        self.assertIn('19', duomo_lines)
